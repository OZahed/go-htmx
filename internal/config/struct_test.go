...
func TestMarshaler_Marshal(t *testing.T) {
	...
	t.Run("want defaults", func(t *testing.T) {
		destination := dest{}
		m := config.NewMarshaler(config.DefaultKeyBuilder, config.DefaultEnvGetter)
		if err := m.Marshal(&destination, "TEST"); (err != nil) != false {
...
func TestMarshaler_Marshal_osSetEnv(t *testing.T) {
	type Config struct {
	...
	t.Run("want defaults", func(t *testing.T) {
		cfg := Config{}
		m := config.NewMarshaler(config.DefaultKeyBuilder, config.DefaultEnvGetter)
		if err := m.Marshal(&cfg, "APP"); (err != nil) != false {
			t.Errorf("Marshaler.Marshal() error = %v, wantErr %v", err, nil)
		}
